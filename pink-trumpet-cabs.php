...
class Pink_Trumpet
{
    const CONST_BASE_URL_WA            = "https://members.cabsweb.org";
...
*/            
    ...
    public function shortcode_wa_event_list( $atts, $content = ""  )
    {
        ...
        // sort by date
        usort($events, function($a, $b) use ($sorted) { // anonymous function
              // compare dates only
              if ($sorted) {
                    return strtotime($a['StartDate']) - strtotime($b['StartDate']);
              } else {
        ...
        foreach ($events as $event)
        {
            ...
            $expired = $today > $sDate;
            $expired_css = ($expired ? "color:#ababab !important" : "");         // gray override
