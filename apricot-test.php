...
<?php
   ...
   $waApiClient = WaApiClient::getInstance();
    ...
    $waApiClient->initTokenByApiKey('ddkt9y2qer1qncditml7lxbayno2yb ');  //FULL ACCESS
...
my_print_r("<pre>contacts: <br>");
my_print_r( $contactsResult );
my_print_r("</pre>");


    $eventsResult = getEventList($account);

my_print_r("<pre>Events: <br>");
my_print_r( $eventsResult );
my_print_r("</pre>");
...
my_print_r("</pre>");       
    ...
    function my_print_r($content, $show = TRUE)
    {
        if ($show) print_r($content);
    }
