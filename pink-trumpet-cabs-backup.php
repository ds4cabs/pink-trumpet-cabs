...
//    private $wa_client;
    ...
    public function __construct()
    {
        ...
        add_filter                  ('wp_title',                array($this, 'plugin_wp_title'));
...
*/
        {
	    return "$event_name yyy";
            if ( $event_name = urldecode(get_query_var("p_eventname")) ) {
                return "$event_name";
            }
        }
        else return "$event_name xxx";

        return $title;
    }


    // MCL: fix this (NOT WORKING RIGHT)
    function plugin_wp_title( $title, $post_id = null )
    {
        if ( strpos($title, self::CONST_EVENT_DETAIL_PAGE_NAME) !== false ||
             strpos($title, self::CONST_SPONSOR_DETAIL_PAGE_NAME) !== false ||
             strpos($title, self::CONST_REGISTRATION_PAGE_NAME) !== false
